...
import com.eitetu.minecraft.server.util.authlib.ProfileLookupCallback;
				...
				try {
					...
					LOGGER.debug(
							...
							new Object[] {
									...
									Integer.valueOf(response.getProfiles().length) });

					Set<String> missing = Sets.newHashSet(request);
					for (GameProfile profile : response.getProfiles()) {
						LOGGER.debug("Successfully looked up profile {}",
								new Object[] { profile });
					...
					for (String name : missing) {
						LOGGER.debug("Couldn't find profile {}",
								new Object[] { name });
						callback.onProfileLookupFailed(new GameProfile(null,
								name), new ProfileNotFoundException("Server did not find the requested profile"));
					}
					try {
						Thread.sleep(100L);
					...
					if (failCount == 3) {
						for (String name : request) {
							LOGGER.debug(
									...
									new Object[] { name });
							callback.onProfileLookupFailed(new GameProfile(
									null, name), e);
						}
					} else {
						try {
