...
public class InsecureTextureException extends RuntimeException {
	...
	public static class WrongTextureOwnerException extends InsecureTextureException {
		...
		public WrongTextureOwnerException(GameProfile expected, UUID resultId,
				String resultName) {
			super("Decrypted textures payload was for another user (expected "
					+ expected.getId() + "/" + expected.getName()
					+ " but was for " + resultId + "/" + resultName + ")");
	...
	public static class OutdatedTextureException extends InsecureTextureException {
		...
		public OutdatedTextureException(Date validFrom, Calendar limit) {
			super("Decrypted textures payload is too old (" + validFrom
					+ ", but we need it to be at least " + limit + ")");
