package com.eitetu.minecraft.server.util.authlib.properties;
	...
	public static class Serializer implements JsonSerializer<PropertyMap>, JsonDeserializer<PropertyMap> {
		public PropertyMap deserialize(JsonElement json, Type typeOfT,
			...
			if (json instanceof JsonObject) {
				...
				for (iter = object.entrySet().iterator(); iter.hasNext();) {
					entry = iter.next();
					if (entry.getValue() instanceof JsonArray) {
						for (JsonElement element : (JsonArray) entry.getValue())
							result.put(entry.getKey(),
									new Property((String) entry.getKey(),
											element.getAsString()));
					}
				}
				...
				for (JsonElement element : (JsonArray) json) {
					if (element instanceof JsonObject) {
						...
						if (object.has("signature"))
							result.put(name, new Property(name, value, object
									.getAsJsonPrimitive("signature")
									.getAsString()));
						else {
