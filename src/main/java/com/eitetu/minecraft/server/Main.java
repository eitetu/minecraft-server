...
public class Main {
	...
	public static void main(String[] args) {
		...
		OptionParser parser = new OptionParser() {
			{
				...
				acceptsAll(
						asList("W", "world-table", "universe", "world-container"), "World container")
						.withRequiredArg()
						.ofType(String.class)
				...
				acceptsAll(asList("C", "commands-settings"), "File for command settings")
						...
						.ofType(String.class)
						.defaultsTo("commands")
						.describedAs("Yml table");

				acceptsAll(asList("nojline"), "Disables jline and emulates the vanilla console");

				acceptsAll(asList("noconsole"), "Disables the console");

				acceptsAll(asList("v", "version"), "Show the eitetu-CraftBukkit Version");
		...
		if ((options == null) || (options.has("?"))) {
			...
			System.out.println(MinecraftServer.class.getPackage().getImplementationVersion());
		} else {
			try {
				// This trick bypasses Maven Shade's clever rewriting of our
				// getProperty call when using String literals
				String jline_UnsupportedTerminal = new String(new char[] { 'j',
						'l', 'i', 'n', 'e', '.', 'U', 'n', 's', 'u', 'p', 'p',
						'o', 'r', 't', 'e', 'd', 'T', 'e', 'r', 'm', 'i', 'n',
						'a', 'l' });
				String jline_terminal = new String(new char[] { 'j', 'l', 'i',
						'n', 'e', '.', 't', 'e', 'r', 'm', 'i', 'n', 'a', 'l' });

				useJline = !(jline_UnsupportedTerminal).equals(System
						.getProperty(jline_terminal));
